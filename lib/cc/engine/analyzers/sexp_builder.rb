  ...
  module Engine
    module Analyzers
      class SexpBuilder
        ...
        def build
          case node
          when CC::Parser::Node then create_sexp(node.type, valid_properties)
          when Array then node.map { |v| self.class.new(v, file, scrub_node_properties: scrub_node_properties).build }
        ...
        attr_reader :node, :file, :scrub_node_properties

        def create_sexp(type, properties)
          sexps = properties.map do |key, value|
            Sexp.new(key.to_sym, *self.class.new(value, file, scrub_node_properties: scrub_node_properties).build).tap do |s|
              s.file = file
